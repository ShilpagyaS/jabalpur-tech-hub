...
socials.splice(2, 0, youtubeEntry);
...
const SocialFloat = () => {
  return (
    <motion.div
      ...
      className="fixed left-4 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-3 hidden md:flex"
    >
      {socials.map(({ label, href, color, hoverShadow, icon }) => (
        <a
          key={label}
          href={href}
          target="_blank"
          ...
          aria-label={label}
          className={`w-11 h-11 flex items-center justify-center rounded-full bg-gradient-to-br ${color} ${hoverShadow} shadow-lg transition-all duration-300 hover:scale-110`}
        >
          {icon}
        </a>
      ))}
